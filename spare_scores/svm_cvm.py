...
from .data_prep import logging_basic_config, prep_spare_cvm
...
class SVMModel_CVM:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        valid_parameters = ["kernel", "k", "n_repeats", "task", "param_grid","scale_features"]
        ...
        if "scale_features" not in kwargs.keys():
    ...
    def train_initialize(self, df: pd.DataFrame, to_predict: str) -> None:
        ...
        self.folds = list(
            ...
            ).split(df[self.key_var],df[self.to_predict])
        )
              
        if len(id_unique) < len(df):
            print('Non-unique key_var found. May cause errors.')
            self.folds = [
                [
        ...
        if self.scale_features==True:
            ...
            print("Not scaling features")
            self.scaler = np.nan * len(self.folds)
    ...
    def prepare_sample(
        ...
        if self.scale_features==True:
            ...
            print("Not scaling features")
            X_train, X_test = df.loc[fold[0], self.predictors], 
            df.loc[fold[1], self.predictors]
        y_train, y_test = (
